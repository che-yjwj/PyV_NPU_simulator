...
- **요약**: `IOBufferTracker` FIFO 동작 개선, 시스템 버스 모델 추가, SPM bank 포트 및 DMA burst 크기 파라미터화 완료. DRAM 채널/페이지 정책 파라미터화 작업 필요.
...
- **내용**: L2 캐시 모듈 구현 및 시스템 통합 완료. PR 리뷰 피드백 반영하여 코드 개선 및 테스트 수정 완료.
...
- **내용**: FIFO 동작, bank/port 충돌, P95 리포팅 등 M-01 과제의 모든 수용 기준을 충족하는 검증 테스트를 추가함.
- **우선순위**: P1 / **사이즈**: L / **의존성**: — / **태그**: 메모리, DMA, 버스
