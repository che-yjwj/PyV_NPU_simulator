...
- **우선순위**: P0 / **사이즈**: M / **의존성**: — / **태그**: 캐시, Py-V, CPU, 아키텍처
...
- **요약**: 시뮬레이터에 비동기 클럭 도메인 지원을 추가하여 CPU, TE, VE가 각기 다른 클럭 속도를 갖도록 설정하는 기능을 구현합니다.
...
### MEM-01 | L0 SPM 모델 구현
- **요약**: 연산 유닛(TE)에 가장 가까운 타일 레벨 L0 SPM을 `resources.py`에 추가합니다.
